...
@Desc  :HashMap的python实现：
HashMap的基本结构：本质上就是一个 元素是链表的定长数组 [LinkedList_1, LinkedList_2...LinkedList_capacity]
由元素的hash值 % 数组长度来决定存放在哪个链表中
"""
...
class HashMap:
    ...
    def reset(self):
        """
        linked_list.size >= self.load_factor * self.capacity
        当有链表长度超过 load_factor * capacity 时，
        把数组的长度扩大一倍，然后把原本的数据一一读取出来，重新hash分配到新的桶当中
        """
